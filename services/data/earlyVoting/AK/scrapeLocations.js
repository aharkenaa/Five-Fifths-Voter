...
async function processRawWebScrape(website) {
  ...
  const filteredSet = querySelectorResultArray.filter(element => {
    if (
      element.cells.length === 3 &&
      element.cells.item(0).innerHTML !== undefined &&
      element.cells.item(0).innerHTML.includes('Early Vote Location')
    ) {
      return true;
    }
    return false;
  ...
  /*
    ...
    */
  const splitSet = filteredSet.map(element =>
    element.cells
      .item(0)
      .innerHTML.split(/<[^>]*>|\n/)
...
async function processLocationArr(locationArr) {
  ...
  for (let i = 0; i < locationArr.length; i += 1) {
    ...
    promiseArr.push(
      fetch(
        `https://maps.googleapis.com/maps/api/geocode/json?address=${currentElement.address}, ${currentElement.county}, AK&key=${process.env.GOOGLE_MAPS_API_KEY}`
      )
        .then(res => res.json())
  ...
  await Promise.all(promiseArr).then(addressList => {
    for (let i = 0; i < addressList.length; i += 1) {
      ...
      const county = currentElement.res.results[0].address_components
        .find(element => element.types[0] === 'administrative_area_level_2')
        .long_name.toUpperCase()
        .replace(' ', '_');
      const key = `${currentElement.location.location.toUpperCase()} ${county} AK ${
        currentElement.res.results[0].address_components.find(
      ...
      if (locationMap.has(county)) {
        ...
        currentEntry[key] = {
          ...
          original_address: `${currentElement.location.location.toUpperCase()} ${county}, AK ${
            currentElement.res.results[0].address_components[6].long_name
          }`,
...
async function saveVotingLocations(locationMap) {
  ...
  locationMap.forEach((value, key) => {
    promiseArr.push(
      fs.writeFile(
        `${__dirname}/knownLocations/${key}.json`,
        JSON.stringify(locationMap.get(key), null, '\t'),
        'utf8',
        err => {
          if (err === null) {
